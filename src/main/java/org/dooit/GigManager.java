...
public class GigManager {
    ...
    public boolean createGig(Gig gig, String documentName) {
        DocumentReference gigDoc = db.collection(COLLECTION_NAME).document(documentName);
    ...
    public List<Gig> getAvailableGigs() {
        ...
        Set<String> seenGigIds = new HashSet<>(); // Track unique gig IDs

        try {
            QuerySnapshot querySnapshot = query.get();
            for (DocumentSnapshot document : querySnapshot.getDocuments()) {
                Gig gig = document.toObject(Gig.class);
                if (gig != null && seenGigIds.add(gig.getGigId())) { // Add only if unique
                    gigList.add(gig);
                }
            }
        } catch (InterruptedException | ExecutionException e) {
